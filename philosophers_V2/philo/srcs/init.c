...
/*   Updated: 2022/10/02 19:23:30 by seungsle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int init_data(int argc, char **argv, t_data *data)
{
    ...
    if (!data->main_lock)
        return (print_error("malloc error") && free_util(data, FREE_MAIN_LOCK, -1));
    if (pthread_mutex_init(data->main_lock, NULL))
        return (print_error("mutex init error") && free_util(data, FREE_MAIN_LOCK, -1));
}
...
int init_forks(t_data *data)
{
    ...
    if (!data->forks)
        return (print_error("malloc error") && free_util(data, FREE_FORKS, -1));
    while (++i < data->num_of_philo)
        if (pthread_mutex_init(&forks[i], NULL))
            return (print_error("mutex init error") && free_util(data, FREE_FORKS, -1));
    return (0);
}
...
int init_philo(t_data *data)
{
    ...
    if (!data->philo)
        return (print_error("malloc error") && free_util(data, FREE_PHILO, -1));
    if (init_forks(data))
        return (1);
    while (++i < data->num_of_philo)
    {
        data->philo[i].id = i;
        data->philo[i].l_fork = &forks[(i + 1) % data->num_of_philo];
        data->philo[i].r_fork = &forks[i];
        data->philo[i].philo_lock = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t));
        if (!data->philo[i].philo_lock)
            return (print_error("malloc error") && free_util(data, FREE_PHILO_LOCK, i));
        if (pthread_mutex_init(data->philo[i].philo_lock, NULL))
            return (print_error("mutex error") && free_util(data, FREE_PHILO_LOCK, i));
